...
![](assets/实战演练_动手实现权限系统-1.jpeg)

经过一段时间后，开发完成。

![](assets/实战演练_动手实现权限系统-2.jpeg)
...
![](assets/实战演练_动手实现权限系统-3.jpeg)

执行规则引擎，生效，拦截下ReadFile(\*.env\*)

执行ReadFile(\*.env\*)

![](assets/实战演练_动手实现权限系统-4.jpeg)
...
![](assets/实战演练_动手实现权限系统-5.jpeg)

调用Glob去读hello.txt

![](assets/实战演练_动手实现权限系统-6.jpeg)

如果Agent 调 WriteFile 或 Bash → **弹出选项** ，按 1 放行 / 2 始终允许 / 3 拒绝

写一个数字1到hello.txt

![](assets/实战演练_动手实现权限系统-7.jpeg)

等我们允许后，才进行文件的写入

![](assets/实战演练_动手实现权限系统-8.jpeg)

对于 **「Accept Edits」模式，** Agent 调 WriteFile/EditFile → **直接执行** ，无弹窗

写一个数字1到hello.txt

![](assets/实战演练_动手实现权限系统-9.jpeg)

如果Agent 调 Bash → **弹出 Modal 对话框**

用Bash写一个数字1到hello.txt

![](assets/实战演练_动手实现权限系统-10.jpeg)

对于 **「Plan」模式，** Agent 调 ReadFile → 正常

读下hello.txt

![](assets/实战演练_动手实现权限系统-11.jpeg)

Agent 调 WriteFile 或 Bash → **直接被拒绝**

写一个数字1到hello.txt

![](assets/实战演练_动手实现权限系统-12.jpeg)
...
![](assets/实战演练_动手实现权限系统-13.jpeg)

写一个数字2到hello.txt

![](assets/实战演练_动手实现权限系统-14.jpeg)

但 `rm -rf /` 等黑名单命令仍然被硬拦截

执行 `rm -rf /`

![](assets/实战演练_动手实现权限系统-15.jpeg)
