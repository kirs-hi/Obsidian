---
title: "《AI大模型Ragent项目》——SSE协议与流式响应"
source: "https://articles.zsxq.com/id_vj6bjkw3zw3w.html"
author:
  - "[[马丁]]"
published:
created: 2026-06-05
...
![无法获取该图片](https://oss.open8gu.com/iShot_2026-03-06_10.13.27.png "无法获取该图片")
...
- **`finish_reason` 只在最后一个 chunk 有值** ，之前的 chunk 都是 `null` 。值不只是 `"stop"` （正常结束），还可能是 `"length"` （达到 max\_tokens 上限被截断）、 `"content_filter"` （被安全过滤截断）、 `"tool_calls"` （模型转入工具调用流程）。代码里需要根据不同的值做相应处理。
