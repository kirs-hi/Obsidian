---
source: "https://news.qq.com/rain/a/20260525A039WF00?adChannelId=tech"
clipped: 2026-05-25
title: "“Token”必须死？_腾讯新闻"
author: "腾讯网"
---

![图片](https://inews.gtimg.com/news_bt/O8DCsjuwhRHa3TU-VYMsDXj_M3bf_3AyAPQVCCg9YnYMoAA/641)

本文为《Token经济学》系列第九期。token正在重塑AI时代的价值坐标，当所有人都在用 token 丈量 AI 的价值时，两篇几乎同时发表的论文提出了一个更根本的问题：语言生成的核心计算，是否必须发生在 token 空间里？如果答案是否定的，那么围绕 token 建立的技术范式、产品叙事和商业模式，都将面临重新定价。
...
这也引出了一个一直被讨论的核心问题：大语言模型能走到AGI（通用人工智能）吗？

2024年12月，OpenAI 前首席科学家Ilya Sutskever在NeurIPS发表主题演讲，他说“预训练即将终结”。2026年3月，图灵奖得主Yann LeCun离开Meta创办AMI Labs，直接宣判“大语言模型路线错了”。

两位深度学习殿堂级的大师，一位选择颠覆自己亲手开启的预训练时代，另一位选择继续践行自己坚守多年的世界模型路线，去赌“LLM的下一个时代”。当然绝对不是当前的模型不好用或没有商业价值，大模型的用户数量及渗透率都在持续增长，产业价值会越来越大。但是从技术路径来看，他们要表达的是：这条路有一个结构性的天花板，这个天花板恰好卡在通往AGI（通用人工智能）的路上。
...
![图片](https://inews.gtimg.com/news_bt/ONX0YNDxigMv8r5ZdxdkLn8e25hV5D91oiRrsdqM7AZO0AA/1000)图片由AI生成
...
![图片](https://inews.gtimg.com/news_bt/Op--G-ecppJBjJQAB3GGrfgza01AG6-H6C2ALOnZqVdqQAA/641)图：ELF仅用32步采样即超越MDLM、Duo等离散模型1024步的生成质量，且未使用蒸馏加速。模型参数105M，训练数据约为同类方法的十分之一。
...
![图片](https://inews.gtimg.com/news_bt/O4AUFTCkvFR_dEg6_sG69nn8vjJO_RJjyUWX3nY2O7jl8AA/641)图：Cola DLM 整体架构图

两篇论文的核心都在表达，token不是语言建模的必要条件。连续空间可以做得更好、更快、更省。

![图片](https://inews.gtimg.com/news_bt/OrTI-SWzh82rUOt6vDizC2qKi9mmpEDtwTCx3YHCsrJ28AA/1000)图：自回归模型逐token生成，每一步不可逆选择一个离散符号，已选token锁定后续所有可能性。

![图片](https://inews.gtimg.com/news_bt/OvUlot8-NDMH515ZC_WribXWFg6YuAu3aXDAN896IiFCAAA/641)图：连续流模型从噪声出发，沿速度场平滑演化到目标嵌入，全程可逆可调，仅在终点映射回文字，ELF论文。

## 03 AI巨头也在质疑“Tokenization”？
...
再往下游推一步，今天整个行业按 token 收费，是因为自回归模型的成本结构极其透明：生成 1000 个 token 就是串行跑 1000 步推理，输入输出的 token 数直接等于算力消耗。
...
DeepSeek又当“价格屠夫”，大模型下一场战争在Token账单里
