...
**项目背景：** 公司硬件供应链（骑行/充电宝/备件）覆盖采购、仓储、物流、维修、交易全链路，涉及 13 个业务子系统。前期已基于 Flink+Kafka+Doris 搭建实时数仓并稳定运行，但随着供应链控制塔从管理层看板升级为一线人员作战工具，实时数据的消费者从少数运营扩展至数百名一线操作人员，原架构暴露出三个瓶颈：Kafka 中间层不可查导致修数需 ODS 全链路重放（耗时数小时）、去重乱序依赖 ROW_NUMBER+Redis+Doris Sequence 多组件链路过长、13 个子系统维度数据分散在三套存储中一致性难保障。基于以上问题，引入 Apache Paimon 构建流式湖仓，对核心链路进行架构升级。
...
> 「我们最初基于 Flink+Kafka+Doris 搭建了供应链实时数仓，这套架构解决了从 0 到 1 的问题——实时数据能力从无到有，核心链路稳定性做到了半年零 P0。后来业务形态发生了一个根本性变化：供应链控制塔从给管理层看报表的'驾驶舱'，变成了一线仓管、物流、维修人员每天盯着干活的'作战台'。这意味着实时数据的消费者从几十个运营变成了几百个一线操作人员，数据的用途从'看趋势'变成了'指导当下每一个动作'——仓管根据实时库存决定补不补货，维修人员根据工单流转状态调整优先级。这对数据平台提出了原架构满足不了的要求：数据出了问题，以前第二天修好就行，现在一线人员实时依赖它做操作决策，修数必须从小时级压缩到分钟级；13 个子系统的数据汇聚到控制塔，排查问题不能再从 Kafka 头开始消费了，中间态数据必须可查；维度数据散在三套系统里导致关联不一致，一线人员看到矛盾数据就不信任系统了。所以我们引入了 Paimon 做架构升级，核心思路是把'状态管理'从计算层下沉到存储层——去重、乱序过滤、聚合这些原来 Flink 用 State 做的事情，现在由 Paimon 的主键表在写入时自动完成；中间层从 Kafka 黑盒换成 Paimon 可查表；维表从三套系统收口到一套。Flink 变成了无状态的计算编排层，修数从重跑全链路变成直接改目标表，链路复杂度和运维成本都大幅下降。」

这段话的核心逻辑是：**业务变化（消费者从管理层变成一线）→ 新要求（修数快/可观测/一致性）→ 原架构不满足 → 引入 Paimon 解决**。先讲"为什么必须升级"再讲"怎么升级"，让面试官感受到你是被业务需求推着做技术决策的，而不是为了用新技术而用新技术。
